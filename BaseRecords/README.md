# 據點資料索引

據點紀錄分成「據點專屬資料」與「共用資料」兩層。各據點只記錄自身的人員、設施、產線、容量與待辦；可跨據點使用的配方、詞條及配種進度統一放在共用子資料集。

## 據點專屬資料

| 據點 | 定位 | 紀錄 |
|---|---|---|
| 第一據點 | 綜合生產、採礦、料理與家畜牧場 | [01-綜合生產與採礦.md](01-綜合生產與採礦.md) |
| 第二據點 | 各類礦物開採、原油、冶煉與素材加工 | [02-礦業與原油.md](02-礦業與原油.md) |
| 第三據點 | 原油、家畜牧場與農作物生產 | [03-原油與牧場.md](03-原油與牧場.md) |
| 第四據點 | 原油、配種、料理與配種食物供應 | [04-油場與配種.md](04-油場與配種.md) |
| 第五據點 | 配種、家畜牧場與農作物生產 | [05-配種.md](05-配種.md) |
| 第六據點 | 新設空據點，用途待規劃 | [06-待規劃.md](06-待規劃.md) |

## 共用子資料集

| 子資料集 | 用途 |
|---|---|
| [據點存檔現況.md](共用資料/據點存檔現況.md) | 由 `Level.sav` 自動產生的所屬公會據點設施、工作帕魯與固定指派現況 |
| [存檔盤點說明.md](共用資料/存檔盤點說明.md) | 所屬公會據點的存檔來源、辨識方式、數量規則與盤點限制 |
| [料理與配種配方.md](共用資料/料理與配種配方.md) | 各據點共用的料理配方、材料換算與安全庫存基準 |
| [牧場清單.md](共用資料/牧場清單.md) | 依帕魯查詢家畜牧場產物與天生初始牧場等級 |
| [素材清單.md](共用資料/素材清單.md) | 依素材反查可生產的牧場帕魯與天生初始牧場等級 |
| [工作詞條配置.md](共用資料/工作詞條配置.md) | 各類工作帕魯的共用詞條配置 |
| [困難塔主隊伍配置.md](共用資料/困難塔主隊伍配置.md) | 玩家挑戰困難塔主的通用隊伍、技能與培育基準 |
| [配種進度.md](共用資料/配種進度.md) | 全據點共用的親代、路線與培育進度 |
| [已完成配種.md](共用資料/已完成配種.md) | 已完成目標曾實際使用或產出的帕魯，供後續配種重複利用 |
| [遠征隊伍配置.md](共用資料/遠征隊伍配置.md) | 全據點共用的遠征隊伍成員與星級配置 |
| [完整配種表](共用資料/配種資料/README.md) | Palworld 1.0／1.0.1 的 299 個配種單位、44,851 筆完整親代組合與離線查詢工具 |

## 五秒快速更新

在儲存庫根目錄執行：

```powershell
.\Commands\update-base-records.cmd
```

- 存檔沒有變更時，指令會核對本機 SHA-256 後使用去識別化快取，目標為 5 秒內完成。
- 存檔有變更時，指令會在本機 Docker 重新解析；目前 68 MB 存檔的完整冷更新約需 10～15 秒，完成後的再次執行會回到 5 秒內。
- 指令會使用 Git 忽略的 `expedition-player.local.json` 辨識目標玩家，以其親建設施所屬的唯一公會篩選據點；不會把伺服器上其他公會的據點寫入正式紀錄。
- 全程不連網、不讀取 `Data/GuiData/token`，正式紀錄與快取都不保存玩家、公會、GUID、座標、帕魯暱稱或權杖資料。
- 如需忽略快取重新盤點，可執行 `.\Commands\update-base-records.cmd -ForceRefresh`。

## 遠征隊伍更新

先將 `expedition-player.example.json` 複製為 Git 會忽略的
`expedition-player.local.json`，並填入目標玩家名稱與玩家 ID；之後在儲存庫根目錄執行：

```powershell
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\update-expedition-records.ps1
```

- 也可用較短的 `.\Commands\update-expedition-records.cmd` 執行同一支 PowerShell 腳本。
- 指令只解析目前 `Level.sav` 的玩家與帕魯角色資料，篩選該玩家名下、暱稱以「遠征」開頭的帕魯。
- 玩家設定只保存在 `expedition-player.local.json`，正式遠征表不會包含玩家名稱、玩家 ID 或個體 GUID。
- `update-expedition-records.ps1`、摘要產生器與範例設定可提交 Git；本機玩家設定已由 `.gitignore` 排除。

## 玩家個人資料查詢

- 查詢「我有幾隻」或玩家個人帕魯時，必須先讀取 Git 忽略的 `expedition-player.local.json`，以玩家名稱與 `PlayerUId` 唯一確認目標玩家；不得把全伺服器、公會或其他玩家的帕魯合併計算。
- 一般帕魯以 `OwnerPlayerUId` 等於本機設定的 `PlayerUId` 為準。若帕魯位於會清空現任擁有者的共享或特殊容器，只能在 `OwnerPlayerUId` 為空且 `OldOwnerPlayerUIds` 含目標玩家時納入；可取得容器關聯時還需交叉驗證。
- 查詢輸出與正式紀錄不得包含玩家名稱、玩家 UID、其他玩家資料、帕魯個體 GUID 或原始存檔內容；玩家身分只保存在本機忽略檔，不得寫入 `AGENTS.md` 或共用資料。

## 維護規則

1. 共用公式或培育結論只在共用子資料集維護一次。
2. 據點檔案只記錄該據點採用的數量、配置與共用規則的例外。
3. 據點若採用不同配置，需在據點檔案寫明覆寫原因，不直接修改共用基準。
4. 新增據點時，先建立新的據點檔案，再引用需要的共用子資料集。
